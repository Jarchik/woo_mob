    ...
    if (!class_exists('ma_connector'))
    {
        ...
        class ma_connector
        {
            ...
            private function check_db() {
                ...
                $is_exists_column = $wpdb->get_results( $wpdb->prepare( "SHOW COLUMNS FROM `{$wpdb->prefix}mobileassistant_push_settings`
                    WHERE FIELD = %s", 'device_unique_id' ) );
                if ( ! $is_exists_column ) {
                    $wpdb->query( "ALTER TABLE `{$wpdb->prefix}mobileassistant_push_settings` ADD COLUMN `device_unique_id` INT(10)" );
                }

                $is_exists_column = $wpdb->get_results( $wpdb->prepare( "SHOW COLUMNS FROM `{$wpdb->prefix}mobileassistant_push_settings`
                    WHERE FIELD = %s", 'status' ) );
                if ( ! $is_exists_column ) {
                    $wpdb->query( "ALTER TABLE `{$wpdb->prefix}mobileassistant_push_settings` ADD COLUMN `status` TINYINT DEFAULT 1" );
                }
                ...
                if ( ! get_option( 'mobassistantconnector_cl_date' ) ) {
                    $wpdb->replace( $wpdb->options, array( 'option_name' => 'mobassistantconnector_cl_date', 'option_value' => time() ) );
                }
            }
