		...
		if (form.length) {
			...
			if (!form.parents(selectorCommentsContainer).length) {
				...
				form.replaceWith(newCommentForm);
				    // Woocommerce star ratings for comments
				...
				jQuery('p.stars a').click(function(){
					...
					jQuery('#rating').val( $star.text() );
					jQuery('p.stars a').removeClass('active');
					$star.addClass('active');
					return false;
				});
				
				jQuery('#review_form #submit').live('click', function(){
					var rating = $('#rating').val();
					
					if ( jQuery('#rating').size() > 0 && !rating && woocommerce_params.review_rating_required == 'yes' ) {
